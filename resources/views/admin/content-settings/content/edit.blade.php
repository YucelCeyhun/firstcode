...
@section('content')
    ...
    @endif
    <h2 class="text-gray-700 text-4xl">Update a Content</h2>
    <form action="{{action('Admin\ContentSettings\ContentController@update',$content)}}" method="post">
        @csrf
        @method('patch')
        <div class="flex items-center mt-6">
            <label class="font-semibold text-gray-600 w-1/2 lg:w-1/6">İçerik Başlığı</label>
            <input type="text" name="title" value="{{old('title',$content->title)}}" autofocus class="form-input"/>
        </div>
        <div class="flex items-center mt-6">
            <label class="font-semibold text-gray-600  w-1/2 lg:w-1/6">İçerik Açıklaması</label>
            <input type="text" name="description" value="{{old('description',$content->description)}}" class="form-input"/>
        </div>
        <div class="flex items-center mt-6">
            <label class="font-semibold text-gray-600  w-1/2 lg:w-1/6">İçerik İçin Taglar</label>
            <input type="text" name="tag" value="{{old('tag',$content->tag)}}" class="form-input"/>
        </div>
        ...
        <div class="mt-16">
            ...
            <div class="editor-wrapper" id="editor-wrapper">
                ...
                <textarea class="editor h-128" name="body" id="editor" spellcheck="false" autocomplete="off">{{old('body',$content->body)}}</textarea>
